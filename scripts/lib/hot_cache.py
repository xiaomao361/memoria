...
from .config import HOT_CACHE_PATH, HOT_CACHE_CAPACITY, PRIV_MEMORIA_JSON_PATH
from .utils import get_utc_timestamp


def _cache_path(private: bool = False) -> Path:
    """根据 private 参数返回对应缓存路径"""
    return PRIV_MEMORIA_JSON_PATH if private else HOT_CACHE_PATH


def read_hot_cache(private: bool = False) -> dict:
    """
    ...
    Args:
        private: 是否读取私密热缓存
    """
    cache_path = _cache_path(private)
    if cache_path.exists():
        try:
            ...
            print(f"ERROR: hot cache read failed{' (private)' if private else ''}: {e}", file=sys.stderr)
            return {}
    return {}
...
def _entries(cache: dict) -> list:
    """提取热缓存中的所有条目（兼容新旧格式）"""
    # entries 索引存在且非空时才用；空列表直接走 fallback
    if "entries" in cache and isinstance(cache["entries"], list) and cache["entries"]:
        result = []
        for mid in cache["entries"]:
            if mid in cache and isinstance(cache[mid], dict):
                result.append(cache[mid])
        return result
    # fallback：直接遍历所有 dict 条目
    return [v for k, v in cache.items()
            if k != "entries" and isinstance(v, dict)]


def write_hot_cache(cache: dict, private: bool = False) -> bool:
    """写入热缓存"""
    try:
        cache_path = _cache_path(private)
        ...
        print(f"ERROR: hot cache write failed{' (private)' if private else ''}: {e}", file=sys.stderr)
        return False
...
def add_to_hot_cache(
    ...
    try:
        ...
        return write_hot_cache(cache, private=private)
    except Exception as e:
        print(f"ERROR: hot cache add failed{' (private)' if private else ''}: {e}", file=sys.stderr)
        return False


def get_from_hot_cache(memory_id: str, private: bool = False) -> dict:
    """从热缓存获取单条"""
    cache = read_hot_cache(private=private)
...
def list_hot_cache(limit: int = None, private: bool = False) -> list[dict]:
    """列出热缓存条目"""
    cache = read_hot_cache(private=private)
...
def get_by_session_id(session_id: str, private: bool = False) -> dict:
    """根据 session_id 查询热缓存"""
    if not session_id:
        return None
    cache = read_hot_cache(private=private)
...
def update_hot_cache_entry(memory_id: str, new_content: str = None, new_tags: list = None, new_links: list = None, private: bool = False) -> bool:
    """更新热缓存条目"""
    try:
        cache = read_hot_cache(private=private)
        ...
        print(f"ERROR: hot cache update failed{' (private)' if private else ''}: {e}", file=sys.stderr)
        return False


def update_last_recalled(memory_id: str, private: bool = False) -> bool:
    """更新记忆的最后访问时间"""
    try:
        cache = read_hot_cache(private=private)
        ...
        print(f"ERROR: update last_recalled failed{' (private)' if private else ''}: {e}", file=sys.stderr)
        return False
...
def update_importance(memory_id: str, delta: float = None, new_score: float = None, private: bool = False) -> bool:
    """
    更新记忆的重要度分数。兼容两种存储格式：
    ...
    try:
        ...
        print(f"ERROR: update_importance failed{' (private)' if private else ''}: {e}", file=sys.stderr)
        return False


def increment_recall_count(memory_id: str, private: bool = False) -> bool:
    """召回时增加召回计数"""
    try:
        cache = read_hot_cache(private=private)
        ...
        print(f"ERROR: increment_recall_count failed{' (private)' if private else ''}: {e}", file=sys.stderr)
        return False


def get_importance_score(memory_id: str, private: bool = False) -> dict:
    """
    获取记忆的重要度详情。
...
def list_by_importance(min_score: float = 0.0, private: bool = False) -> list[dict]:
    """按重要度筛选记忆"""
    cache = read_hot_cache(private=private)
    results = []
    for entry in _entries(cache):
