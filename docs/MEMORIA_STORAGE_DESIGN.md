...
同一 session 内多次"记一下"，避免重复记录。

### 6.2 方案：session-id 判断

```
新写入请求
    ↓
携带 session-id？
    ├→ 是 → 查询热缓存是否有该 session 的已有记忆
    │       ├→ 有 → 增量更新（追加内容）
    │       └→ 无 → 新建
    └→ 否 → 新建
```
...
增量更新逻辑**内嵌在 store() 内部**，而非独立层。

```
追加到已有记忆：
  1. 追加到 archive TXT（正文末尾追加 --- 分隔 + 新内容）
  2. 删除旧向量，重新写入（合并后的完整内容）
  3. 更新热缓存（timestamp、tags、links）
  4. 合并 links（去重）
  5. version 在 front matter 中递增
```
